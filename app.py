...
# --- Page configuration MUST be set before heavy imports (prevents odd failures on Cloud) ---
st.set_page_config(
...
# --- Data generator (cached) ---
@st.cache_data
def generate_sleep_data(days: int) -> pd.DataFrame:
    dates = pd.date_range(end=datetime.now(), periods=days, freq="D")
    rng = np.random.default_rng(42)

    duration = rng.uniform(6, 9, days)
    deep_sleep = rng.uniform(1, 2.5, days)
    rem_sleep = rng.uniform(1.5, 2.5, days)
    light_sleep = rng.uniform(3, 5, days)
    awake = rng.uniform(0.2, 0.7, days)
    quality = rng.uniform(60, 95, days)
    heart_rate = rng.uniform(55, 70, days)
    bed_time = rng.uniform(22, 24, days)
    wake_time = rng.uniform(6, 8, days)
...
with st.sidebar:
    ...
    time_range = st.selectbox("Select Time Range", ["Week", "Month", "Quarter"], index=0)
    ...
    st.markdown(
        ...
        3. *Generate Insights*: Practical recommendations  
...
with c1:
    st.metric(label="⏰ Avg Sleep Duration", value=f"{avg_duration:.1f}h", delta="+0.3h")

with c2:
    st.metric(label="⭐ Sleep Quality", value=f"{avg_quality:.0f}%", delta="+5%")

with c3:
    st.metric(label="🧠 Deep Sleep", value=f"{avg_deep_sleep:.1f}h", delta="+0.2h")

with c4:
    st.metric(label="❤ Resting Heart Rate", value=f"{avg_heart_rate:.0f} bpm", delta="-2 bpm")
...
with b3:
    ...
    fig5.update_layout(
        ...
        yaxis=dict(
            showgrid=True, gridcolor="rgba(255,255,255,0.1)", title="Hour", range=[0, 24]
        ),
        height=300,
...
with i1:
    st.info(
        ...
        *✨ Sleep Consistency*
        ...
        Try maintaining consistent bed and wake times for better sleep quality and
...
with i2:
    st.success(
        ...
        *🎯 Deep Sleep Optimization*
        ...
        You're averaging {avg_deep_sleep:.1f}h of deep sleep per night. A typical
        optimal range is around 1.5–2h. Consider reducing screen time before bed
        and keeping your room cool to boost deep sleep.
        """
    )

# --- Data table (requires matplotlib for background_gradient) ---
